...
\icmlkeywords{Higher-order tensors, ordinal data, tensor decomposition, tensor completion}
...
%{\color{red}In contrast to the continuous-valued problems, parameter estimation with ordinal observation exhibit a phase transition depending on the signal-to-noise ratio. (Not sure whether this should be mentioned)}
...
(P2) Given an incomplete set of ordinal observation $\{y_{\omega}\}_{\omega\in\Omega}$ based on the model~\eqref{eq:model}, how many sampled entries do we need to consistently recover $\Theta$? 
...
\begin{algorithm}[tb]
        \caption{Ordinal tensor decomposition ({\color{red} allowing missing data})}\label{alg}
        \begin{algorithmic}[]
            \STATE{\bfseries Input:}  \text{ Ordinal tensor }
            $\mathcal{Y}\in [L]^{d_1\times\cdots\times d_K}
            $,\\ \hspace{.43in} Rank $\mr\in \mathbb{N_+}^{K}$,\\
...
&=\argmin_{\mr\in\mathbb{N}^K_{+}}\left[-2\tL_{\tY}(\hat \Theta(\mr),\hat \mb)+p_e(\mr)\log (\prod_k d_k) \right],
\end{align}
...
\section{Experiments}
In this section, we evaluate the empirical performance of our estimation method. We consider both the complete and the incomplete settings, and compare the recovery accuracy of our estimator with other tensor-based methods. Unless otherwise stated, we generated $L$-level ordinal data tensors from model~\eqref{eq:model} using standard probit link $f$. We consider order-3 parameter tensors with $d_1=d_2=d_3=d$ and $r_1=r_2=r_3=r$. The parameter tensor is simulated based on~\eqref{eq:Tucker}, where the entries in the core tensor $\tC$ are i.i.d.\ drawn from $N(0,1)$, and the factors $\mM_k$ are uniformly sampled (with respect to Haar measure) from matrices with orthonormal columns. We set the cut-off points $b_\ell=f^{-1}(\ell/L)$ for $\ell=0,\ldots,L$, such that $f(b_\ell)$ are evenly spaced from 0 to 1. In each simulation study, we report the summary statistics across $n_{\text{sim}}$ = 30 replications. 

\subsection{Finite-sample performance}\label{sec:simulation}
We first consider the case of complete observation, and assess the empirical relationship between the MSE and model complexity. In particular, we consider various settings of dimension $d$, rank $r$ and signal level $\alpha=\mnormSize{}{\Theta}$. 
Figure~\ref{fig:finite}a plots the estimation error versus the tensor dimension $d$ for three different ranks $r\in\{3,5,8\}$. The decay in the error appears to behave on the order of $d^{-2}$, which is consistent with our theoretical results~\eqref{eq:rate}. It is seen that a higher rank leads to a larger error, as reflected by the upward shift of the curve as $r$ increases. Indeed, a higher rank implies a higher intrinsic dimension, thus increasing the difficulty of the estimation. 

Figure~\ref{fig:finite}b shows the estimation error versus the signal level. A larger estimation error is observed when the signal is either too small or too large. This non-monotonic phenomenon may seem surprising, but we believe this is an intrinsic feature with ordinal-valued date problem. In light of the latent-variable interpretation (see Section~\ref{sec:latent}), estimation from ordinal observation can be interpreted as an inverse problem of quantization. The estimation error will diverge when the noise $\tE$ is negligible, because there is no way to distinguish two different signal tensors, say, $\Theta_1=\ma_1\otimes \ma_2\otimes \ma_3$ and $\Theta_2=\text{sign}(\ma_1)\otimes \text{sign}(\ma_2)\otimes \text{sign}(\ma_3)$, from the quantized observations. This behavior is clearly contrary to that of classical continuous-valued tensor problem, and the phenomenon has attracted great interest recently~\cite{davenport2014,sur2018modern}. 


We then investigate the case of incomplete observation. We sample tensor entries via uniform sampling and assess the recovery error with sample complexity. Figure~\ref{fig:finite}c shows the completion error versus the sample size. The error reduces roughly at a rate of $\tO(d^{-2})$(?) as the sample size increases. This suggests....
...
\includegraphics[width=8.3cm]{panel.pdf}
\vspace{-.4cm}
\caption{Finite sample performance}\label{fig:finite}
\end{figure}
\vspace{-.5cm}

%\begin{figure}[http]
%\includegraphics[width=4.2cm]{error_level.pdf}
%\end{figure}
...
Compare with 
\begin{enumerate}
\item continuous tensor method~\cite{filipovic2015tucker}; 
\item 1-bit tensor method~\cite{ghadermarzy2018learning}; 
\item ordinal matrix method~\cite{bhaskar2016probabilistic}. 
\end{enumerate}
...
\midrule
\begin{tabular}[c]{@{}c@{}}Oridnal tensor\\ Decomposition\end{tabular}  & 0.1504 & 0.1503& 0.1502 \\ \\
\begin{tabular}[c]{@{}c@{}}Conti tensor\\ Decomposition\end{tabular} & 0.1604& 0.1600& 0.1598\\ \\
\begin{tabular}[c]{@{}c@{}}1bit-completion\\ (probit)\end{tabular}  & 0.3658 & 0.3633 & 0.3620\\
\begin{tabular}[c]{@{}c@{}}1bit-completion\\ (logistic)\end{tabular}  &  0.4519& 0.4219 & 0.4068
\\
\bottomrule
\end{tabular}
\end{sc}
...
%\hline
%MSE& 0.1504&  0.1504 & 0.1504&0.1604\\
%MAD& 0.1503&0.1503 &0.1503&0.1600\\
%MCR& 0.1503& 0.1502&0.1502& 0.1598\\
%\hline
%\end{tabular}
%}
%\caption{Comparison of tensor decomposition on HCP dataset.}
%\end{table}
