...
We also compare the goodness-of-fit of various tensor methods on the HPC data. We perform 5-fold cross-validation while preserving the label proportions in train/test sets. Table~\ref{table:CV} summarizes the prediction error averaged over 10 runs. Our method outperforms the others, especially in MAD.
...
\[
\mathrm{MSE}(\hat \Theta, \trueT)\leq \min\left\{ 4\alpha^2,\ {c_1U_{\alpha,\beta,\Delta}\over L_{\alpha,\beta,\Delta}} r_{\max}^{(K-1)/2}  \sqrt{ \sum_k d_k \over \prod_k d_k}\right\},
\]
and
\[
\text{MSE}(\hat\mb, \trueb) \leq  \min\left\{ 4\beta^2,\ {c_2 C_{\alpha,\beta,\Delta} \over D_{\alpha,\beta,\Delta}}{\max_{\ell}(n_\ell+n_{\ell+1})\over \min_{\ell}(n_{\ell}+n_{\ell+1})}  \sqrt{1\over L-1}\right\},
\]
...
The constant bounds $4\alpha^2$ and $4\beta^2$ follow trivially from the definition of the feasible sets. We now focus on proving the non-constant bounds. Similar to the proof of Theorem~\ref{thm:rate}, we suppress $\Omega$ in the subscript. Based on the definition of $(\hat \Theta, \hat \mb)$,
...
Table~\ref{table:clustering} shows the eight clusters and their compositions. We find that most brain nodes fall into cluster I and the cluster II, which can be represented as the left and right hemispheres, respectively (Figure~\ref{figure:brain image}). Clusters III-VIII captures brain regions with same annotations in the dataset. For example, clusters III/IV represents the supramarginal gyrus region on the left/right hemisphere, respectively. This region is known to be involved in visual word recognition and reading~\citep{stoeckel2009supramarginal}. The results demonstrate that our clustering successfully groups similar brain regions together without knowledge of external annotation.
...
\begin{figure}[H]
 ...
 \includegraphics[width = 0.45\textwidth]{braincluster3.pdf}
  \caption{Top two clusters of brain nodes. (a) Nodes in yellow are from cluster I (left hemisphere), and nodes in green are from cluster II (right hemisphere). (b) Predicted connectivity within cluster I and cluster II. Edges are colored based on predicted strength level averaged across individuals.}  \label{figure:brain image}
\end{figure}
