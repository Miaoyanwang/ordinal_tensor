...
%DIF DEL ordinalT_arxiv_v1.tex   Wed Feb 12 20:58:37 2020
%DIF ADD ordinalT_arxiv_v2.tex   Thu Feb 13 00:25:11 2020
...
%DIF < 
%DIF < 
%DIF < \renewcommand{\textfraction}{0.0}
%DIF < \renewcommand{\topfraction}{1.0}
%DIF < %\renewcommand{\textfloatsep}{5mm}
%DIF < 
%DIF -------
...
\usepackage{comment}
%DIF 19d9
%DIF < % Definitions of handy macros can go here
%DIF -------
\usepackage{amsmath,amssymb,amsthm,bm,mathtools}
\usepackage{natbib}
%DIF 22d11
%DIF < %\usepackage{dsfont,multirow,hyperref,setspace,natbib,enumerate}
%DIF -------
...
%DIF 29c17-18
%DIF < \mathtoolsset{showonlyrefs=true}
%DIF -------
...
%DIF 39a26
\newtheorem{cor}{Corollary}[section] %DIF > 
%DIF -------
...
\newcommand{\cmark}{\ding{51}}% %DIF > 
\newcommand{\xmark}{\ding{55}}% %DIF > 
%DIF -------
...
%DIF < 
%DIF < 
%DIF < \def\MLET{\hat \Theta_{\text{MLE}}}
...
%DIF < 
%DIF < 
%DIF < \DeclareMathOperator*{\minimize}{minimize}
%DIF < 
%DIF -------
\usepackage{wrapfig}
%DIF 63d40
...
%DIF 75-76c52-62
%DIF < \usepackage{xr}
%DIF < \externaldocument{ordinalTv3}
%DIF -------
\begingroup %DIF > 
    \makeatletter %DIF > 
    \@for\theoremstyle:=definition,remark,plain\do{% %DIF > 
        \expandafter\g@addto@macro\csname th@\theoremstyle\endcsname{% %DIF > 
            \addtolength\thm@preskip\parskip %DIF > 
            }% %DIF > 
        } %DIF > 
\endgroup %DIF > 
 %DIF > 
...
Multidimensional arrays, a.k.a.\ tensors, arise in a variety of applications including recommendation systems~\citep{baltrunas2011incarmusic}, social networks~\citep{nickel2011three}, genomics~\citep{hore2016tensor}, and neuroimaging~\citep{zhou2013tensor}. There is a growing need to develop general methods \DIFaddbegin \DIFadd{that can handle two main problems }\DIFaddend for analyzing these noisy, high-dimensional datasets\DIFdelbegin \DIFdel{that can handle two main problems. The problem of tensor denoising -- }\DIFdelend \DIFaddbegin \DIFadd{. The first problem is tensor denoising }\DIFaddend which aims to recover a signal tensor from its noisy entries\DIFdelbegin \DIFdel{-- has gained increased attention in theory and applications~\mbox{%DIFAUXCMD
\citep{xia2019sup,zeng2019multiway}}%DIFAUXCMD
. A related problem , tensor completion , }\DIFdelend \DIFaddbegin \DIFadd{~\mbox{%DIFAUXCMD
\citep{hong2018generalized,zeng2019multiway}}%DIFAUXCMD
. A second problem is tensor completion which }\DIFaddend examines the minimum number of entries needed for a consistent recovery~\citep{ghadermarzy2018learning,ghadermarzy2019near}. Low-rankness is often imposed to the signal tensor, which efficiently reduces the intrinsic dimension in both problems.

A number of low-rank tensor estimation methods have been proposed~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{kolda2009tensor,acar2010scalable}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{acar2010scalable,anandkumar2014tensor}}%DIFAUXCMD
}\DIFaddend , revitalizing classical methods such as CANDECOMP/PARAFAC (CP) decomposition~\citep{hitchcock1927expression} and Tucker decomposition~\citep{tucker1966some}. These tensor methods treat the entries as continuous-valued. In many cases, however, we encounter datasets of which the entries are qualitative. For example, the Netflix problem records the ratings of users on movies over time. Each data is a rating on a nominal scale \{{\it very like, like, neutral, dislike, very dislike}\}. Another example is in the signal processing, where the digits are frequently rounded or truncated so that only integer values are available. The qualitative observations take values in a limited set of categories, making the learning problem harder compared to  continuous observations.

 Ordinal entries are categorical variables with an ordering among the categories; for example, {\it very like} $\prec$ {\it like} $\prec$ {\it neutral} $\prec \cdots$. The analyses of tensors with the ordinal entries are mainly complicated by two key properties needed for a reasonable model. First, the model should be invariant under a reversal of categories, say, from the Netflix example, {\it very like} $\succ$ {\it like} $\succ$ {\it neutral} $\succ \cdots$, but not under arbitrary label permutations. Second, the parameter interpretations should be consistent under merging or splitting of contiguous categories. The classical continuous tensor model~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{kolda2009tensor, ghadermarzy2019near} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{kolda2009tensor, ghadermarzy2019near} }%DIFAUXCMD
}\DIFaddend fails in the first aspect, whereas the binary tensor model~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{ghadermarzy2018learning} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{ghadermarzy2018learning} }%DIFAUXCMD
}\DIFaddend lacks the second property. An appropriate model for ordinal tensors has yet to be studied.

\DIFdelbegin %DIFDELCMD < \begin{table*}[ht]
%DIFDELCMD < \centerline{\includegraphics[width=16cm]{compare.pdf}}
%DIFDELCMD < %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Comparison with previous work. For ease of presentation, we summarize the error rate and sample complexity assuming equal tensor dimension in all modes. $K$: tensor order; $L$: number of ordinal levels; $d$: dimension at each mode. }}%DIFAUXCMD
\DIFdelFL{~}%DIFDELCMD < %DIFDELCMD < \label{tab:compare}%%%
%DIFDELCMD < \end{table*}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend {\bf Our contribution.} This paper presents an efficient low-rank estimation method and theory for tensors with ordinal-valued entries. Our main contributions are summarized in Table~\ref{tab:compare}. We propose a cumulative link model for higher-order tensors, develop a rank-constrained M-estimator, and obtain theoretical accuracy guarantees. The mean squared error bound is established, and we show that the obtained bound has minimax optimal rate in high dimensions under the low-rank model. Our estimator enjoys a faster convergence rate $\tO(d^{-(K-1)/2})$ than $\tO(d^{-K})$ in~\citet{ghadermarzy2018learning}, which is a substantial improvement as the order $K$ increases. Furthermore, our proposal serves as an efficient completion algorithm that guarantees consistent recovery of an order-$K$ $(d,\ldots,d)$-dimensional low-rank tensor using only $\tilde \tO(Kd)$ noisy, quantized observations.
...
\caption{\DIFaddFL{Comparison with previous work when tensor rank $r=O(1)$ (neglecting log factors). For ease of presentation, we summarize the error rate and sample complexity assuming equal tensor dimension in all modes. $K$: tensor order; $L$: number of ordinal levels; $d$: dimension at each mode. }}\DIFaddFL{~}\label{tab:compare}
\end{table}


\DIFaddend {\bf Related work.} Our work is related to, but clearly distinctive from, several lines of existing literature. Matrix completion from quantized samples was firstly introduced for binary observations~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{cai2013max,davenport2014,bhaskar20151} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{cai2013max,davenport2014,bhaskar20151} }%DIFAUXCMD
}\DIFaddend and then extended to ordinal observations~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{bhaskar2016probabilistic}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{bhaskar2016probabilistic}}%DIFAUXCMD
}\DIFaddend . As we show in Section~\ref{sec:theory}, applying existing matrix methods to an ordinal tensor results in a suboptimal estimator with a slower convergence rate. Therefore, a full exploitation of the tensor structure is necessary; this is the focus of the current paper.

Our work is also connected to non-Gaussian tensor decomposition. Existing work focuses exclusively on univariate observations such as binary- or continuous-valued entries~\citep{wang2018learning,hong2018generalized,ghadermarzy2018learning}. As we mentioned earlier, the ordinal observations add considerable challenges to the model formulation. We address the problems from two perspectives. From statistical perspective, our proposed model generalizes the usual binary tensor model while preserving palindromic invariance~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{mccullagh1980regression} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{mccullagh1980regression} }%DIFAUXCMD
}\DIFaddend for ordinal observations. From algorithm perspective, our alternating optimization compares favorably to the approximate (non-convex) algorithm developed in the context of binary tensors~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{ghadermarzy2018learning}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{ghadermarzy2018learning}}%DIFAUXCMD
}\DIFaddend . We numerically compare the two approaches in Section~\ref{sec:experiment}.


\section{Preliminaries}
Let $\tY\in\mathbb{R}^{d_1\times \cdots \times d_K}$ denote an order-$K$ $(d_1,\ldots,d_K)$-dimensional tensor. We use $y_\omega$ to denote the tensor entry indexed by $\omega$, where $\omega\in[d_1]\times\cdots\times[d_K]$.  The Frobenius norm of $\tY$ is defined as $\FnormSize{}{\tY}=\sum_\omega y^2_\omega$ and the infinity norm of $\tY$ is defined as $\mnormSize{}{\tY}=\max_{\omega}|y_\omega|$. We use $\tY_{(k)}$ to denote the unfolded matrix of size $d_k$-by-$\prod_{i\neq k}d_k$, obtained by reshaping the tensor along the mode-$k$. The Tucker rank of $\tY$ is defined as a length-$K$ vector $\mr = (r_1,\ldots,r_K)$, where $r_k$ is the rank of matrix $\tY_{(k)}$ for all $k \in[K]$. We say that an event $A$ occurs ``with very high probability'' if $\mathbb{P}(A)$ tends to 1 faster than any polynomial of tensor dimension $d_{\min}=\min\{d_1,\ldots,d_K\} \to\infty$. \DIFaddbegin \DIFadd{For any two functions $f, g$ depending on $(d_1,\ldots,d_K)$, we write $f=\tO(g)$ to indicate that $f\leq Cg$, where $C>0$ is a constant independent of tensor dimension. We write $f=\tilde\tO(g)$ to indicate that $f\leq C(\log d_{\min})^\beta g$ for some $\beta>0$. 
}\DIFaddend 
...
\DIFaddend \begin{example}[Probit model] The probit model is characterized by~\eqref{eq:model} with
...
\DIFaddend Other link functions are also possible, such as Laplace, Cauchy, \DIFdelbegin \DIFdel{etc~\mbox{%DIFAUXCMD
\cite{mccullagh1980regression}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{inverse log-log, etc~\mbox{%DIFAUXCMD
\citep{mccullagh1980regression}}%DIFAUXCMD
}\DIFaddend . All the models share the property that the ordered categories can be thought of as contiguous interval on some continuous scale. We should point out that, although the latent-variable interpretation is incisive, our estimation procedure does not refer to the existence of $\tY^*$. Therefore, our model~\eqref{eq:model} is general and still valid in the absence of quantization process. More generally, we make the following assumptions about the link $f$.

\begin{assumption}\label{ass:link}
The link function is assumed to satisfy:
 \begin{enumerate} 
\item $f(\theta)$ is strictly increasing and twice-differentiable in \DIFdelbegin \DIFdel{$\theta\in\mathbb{R}/\{0\}$}\DIFdelend \DIFaddbegin \DIFadd{$\theta$}\DIFaddend .
...
\DIFaddbegin {\bf \DIFaddend (P1)\DIFaddbegin } \DIFaddend Given the quantization process induced by $f$ and the cut-off points $\mb$, how accurately can we estimate the latent signal tensor $\Theta$ from the ordinal observation $\tY$?

Clearly, the problem (P1) cannot be solved uniformly for all possible $\Theta$. We focus on a class of ``low-rank'' and ``flat'' signal tensors, which is a plausible assumption in practical applications~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{zhou2013tensor,bhaskar20151}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{zhou2013tensor,bhaskar20151}}%DIFAUXCMD
}\DIFaddend . Specifically, we consider the parameter space:
\begin{equation}\label{eq:space}
 \DIFdelbegin %DIFDELCMD < \small %%%
\DIFdelend \tP=\left\{\Theta\in\mathbb{R}^{d_1\times \cdots \times d_K} \colon \text{rank}(\Theta)\leq \mr,\ \mnormSize{}{\Theta}\leq \alpha\right\}.
\end{equation}
...
\end{equation}
where $\tC\in\mathbb{R}^{r_1\times \cdots \times r_K}$ is a core tensor, $\mM_k\in\mathbb{R}^{d_k\times r_k}$ are factor matrices with orthogonal columns, and $\times_k$ denotes the tensor-by-matrix multiplication~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{kolda2009tensor}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{kolda2009tensor}}%DIFAUXCMD
}\DIFaddend . The Tucker low-rankness is popularly imposed in tensor data analysis, and is shown to provide a reasonable tradeoff between model complexity and model flexibility. Note that, unlike matrices, there are various notations of tensor low-rankness, such as CP rank~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{hitchcock1927expression} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{hitchcock1927expression} }%DIFAUXCMD
}\DIFaddend and train rank~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{oseledets2011tensor}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{oseledets2011tensor}}%DIFAUXCMD
}\DIFaddend . Some notation of low-rankness may lead to mathematically ill-posed optimization; for example, the best low CP-rank tensor approximation may not exist~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{de2008tensor}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{de2008tensor}}%DIFAUXCMD
}\DIFaddend . We choose Tucker representation for well-posedness of optimization and easy interpretation.
...
\DIFaddbegin {\bf \DIFaddend (P2)\DIFaddbegin } \DIFaddend Given an incomplete set of ordinal observations $\{y_{\omega}\}_{\omega\in\Omega}$, how many sampled entries do we need to consistently recover $\Theta$ based on the model~\eqref{eq:model}?
...
\logl(\Theta, \mb)&=\sum_{\omega\in\Omega}\sum_{\ell\in[L]} \Big\{\mathds{1}_{\{y_\omega=\ell\}} \log \big[f(b_\ell-\theta_\omega)-  \DIFdelbegin %DIFDELCMD < \notag \\
%DIFDELCMD < &\quad \quad \quad \quad \quad \quad \quad  %%%
...
\DIFdelbegin \begin{align*}\DIFdel{%DIFDELCMD < \label{eq:regular}%%%
...
\DIFdelbegin \begin{displaymath}\DIFdel{%DIFDELCMD < \label{eq:ours}%%%
...
Theorem~\ref{thm:completion} shows that our estimator achieves consistent recovery using as few as $\tilde{\tO}(Kd)$ noisy, quantized observations from an order-$K$ $(d,\ldots,d)$-dimensional tensor. Note that $\tilde \tO(Kd)$ roughly matches the degree of freedom for an order-$K$ tensor of fixed rank $\mr$, suggesting the optimality of our sample requirement. This sample complexity substantially improves over earlier result $\tO(d^{\lceil K/2\rceil})$ based on square matricization~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{mu2014square}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{mu2014square}}%DIFAUXCMD
}\DIFaddend , or $\tO(d^{N/2})$ based on tensor nuclear-norm regularization~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{yuan2016tensor}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{yuan2016tensor}}%DIFAUXCMD
}\DIFaddend . Existing methods that achieve $\tilde \tO(Kd)$ sample complexity require either a deterministic cross sampling design~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{zhang2019cross} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{zhang2019cross} }%DIFAUXCMD
}\DIFaddend or univariate measurements~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{ghadermarzy2018learning}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{ghadermarzy2018learning}}%DIFAUXCMD
}\DIFaddend . Our method extends the conclusions to multi-level measurements under a broader class of sampling schemes.
...
%DIFDELCMD <              %%%
\DIFdelend s.t. $\mnormSize{}{\Theta^{(t+1)}}\leq \alpha$, where $\Theta^{(t+1)}$ is the parameter tensor based on the current block estimates.
            \ENDFOR
            ...
            \STATE {Update $\mb^{(t+1)}$ while fixing $\Theta^{(t+1)}$:} 
              \DIFdelbegin %DIFDELCMD < \newline
%DIFDELCMD <               %%%
\DIFdelend $\mb^{(t+1)} \gets \argmax_{\mb\in \tB}\mathcal{L}_{\mathcal{Y},\Omega}\big(\Theta^{(t+1)},\mb\big).$
            \ENDFOR
            \STATE \textbf{return}
            $(\hat \Theta,\hat \mb)$
        \end{algorithmic}
...
Next, we compare our ordinal tensor method ({\bf Ordinal-T}) with three popular low-rank methods:
 ...
 \begin{itemize} 
\item Continuous tensor decomposition ({\bf Continuous-T})~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{acar2010scalable} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{acar2010scalable} }%DIFAUXCMD
}\DIFaddend is a low-rank approximation method based on classical Tucker model.
\item One-bit tensor completion ({\bf 1bit-T})~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{ghadermarzy2018learning} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{ghadermarzy2018learning} }%DIFAUXCMD
}\DIFaddend is a max-norm penalized tensor learning method based on partial binary observations.
\item Ordinal matrix completion ({\bf Ordinal-M})~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{bhaskar2016probabilistic} }%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{bhaskar2016probabilistic} }%DIFAUXCMD
}\DIFaddend is a rank-constrained matrix estimation method based on noisy, quantized observations.
 \end{itemize} 
...
\DIFaddend We evaluate the methods by their capabilities in predicting the most likely label for each entry, i.e.,\ $y_\omega^{\text{mode}}=\arg\max_\ell\mathbb{P}(y_\omega=\ell)$. Two performance metrics are considered: mean absolute deviation, \DIFdelbegin \DIFdel{$\text{MAD}=d^{-K}\sum_\omega |y_\omega^{\text{mode}}-\hat y_\omega^{\text{mode}}|$}\DIFdelend \DIFaddbegin \DIFadd{$\text{MAD}(\tY^{\text{mode}}, \hat \tY^{\text{mode}})=d^{-K}\sum_\omega |y_\omega^{\text{mode}}-\hat y_\omega^{\text{mode}}|$}\DIFaddend , and misclassification rate, \DIFdelbegin \DIFdel{$\text{MCR}=d^{-K}\sum_\omega\mathds{1}_{\{y_\omega^{\text{mode}}\neq\text{round}(\hat y_\omega^{\text{mode}})\}}$}\DIFdelend \DIFaddbegin \DIFadd{$\text{MCR}(\tY^{\text{mode}}, \hat \tY^{\text{mode}})=d^{-K}\sum_\omega\mathds{1}_{\{y_\omega^{\text{mode}}\neq\text{round}(\hat y_\omega^{\text{mode}})\}}$}\DIFaddend ,
where $\text{round}(\cdot)$ denotes the nearest integer of the prediction (possibly continuous-valued returned by {\bf Continuous-T}). \DIFaddbegin \DIFadd{Both metrics are widely used for evaluation of prediction accuracy. }\DIFaddend Note that MAD penalizes the large deviation more heavily than MCR. 
...
\caption{Performance comparison \DIFdelbeginFL \DIFdelFL{in MCR }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{for predicting most likely labels. }\DIFaddendFL (a, \DIFdelbeginFL \DIFdelFL{b) and MAD (}\DIFdelendFL c\DIFdelbeginFL \DIFdelFL{, d}\DIFdelendFL ) \DIFaddbeginFL \DIFaddFL{Prediction errors versus sample complexity $\rho=|\Omega|/d^K$ when $L=5$}\DIFaddendFL . (b, d) Prediction errors versus the number of ordinal levels $L$ when $\rho=0.8$. \DIFdelbeginFL \DIFdelFL{(a, c) Prediction errors versus sample complexity $\rho=|\Omega|/d^K$ when $L=5$. }\DIFdelendFL }\label{fig:compare}
\end{figure}

We also compare the methods by their performance in predicting the median \DIFdelbegin \DIFdel{label}\DIFdelend \DIFaddbegin \DIFadd{labels}\DIFaddend , $y_\omega^{\text{median}}=\min\{\ell\colon\mathbb{P}(y_\omega=\ell)\geq 0.5\}$. Under the latent variable model~\eqref{eq:latent} and Assumption~\ref{ass:link}, the median label is the quantized \DIFdelbegin \DIFdel{$\Theta$ }\DIFdelend \DIFaddbegin \DIFadd{$\theta_\omega$ }\DIFaddend without noise; i.e.\ $y_\omega^{\text{median}}=\sum_\ell \mathds{1}_{\theta_\omega\in(b_{\ell-1},b_\ell]}$. \DIFdelbegin \DIFdel{Similar results demonstrate the outperformance of our method (see the Supplement). }\DIFdelend \DIFaddbegin \DIFadd{We utilize the same simulation setting as in the earlier experiment. Figure~\ref{fig:compare2} shows that our method outperforms the others in both MCR and MAD. The improved accuracy comes from the incorporation of multilinear low-rank structure, the use of multi-level observations, and the preservation of ordinal structure. Interestingly, for the three multilevel methods (}{\bf \DIFadd{1bit-sign-T}}\DIFadd{, }{\bf \DIFadd{Ordinal-M}}\DIFadd{, and }{\bf \DIFadd{Ordinal-T}}\DIFadd{), the median estimator tends to yield smaller MAD than the mode estimator, $\text{MAD}(\tY^{\text{median}}, \hat \tY^{\text{median}})\leq \text{MAD}(\tY^{\text{mode}}, \hat \tY^{\text{mode}})$  (Figures~\ref{fig:compare}a-b vs.\ Figures~\ref{fig:compare2}a-b). On the other hand, the mode estimator tends to yield smaller MCR than the median estimator, $\text{MCR}(\tY^{\text{mode}}, \hat \tY^{\text{mode}})\leq\text{MCR}(\tY^{\text{median}}, \hat \tY^{\text{median}})$ (Figures~\ref{fig:compare}c-d vs.\ Figures~\ref{fig:compare2}c-d). This tendency is from the property that the median estimator $\hat y^{(\text{median})}_\omega$ minimizes $g_1(z)=\mathbb{E}_{y_\omega}|y_\omega-z|$, whereas the mode estimator $\hat y^{(\text{mode})}_\omega $ minimizes $g_2(z)=\mathbb{E}_{y_\omega}\mathds{1}_{\{y_\omega=z\}}$. Here the expectation is over the categorical distribution of $y_\omega$ given parameters $\hat \Theta$ and $\hat \mb$. 
}\DIFaddend 
...
\end{center}
\caption{\DIFaddFL{Performance comparison for predicting median labels. (a,c) Prediction errors versus sample complexity $\rho=|\Omega|/d^k$ when $L=5$. (b,d) Prediction errors versus the number of ordinal levels $L$, when $\rho=0.8.$}}\label{fig:compare2}
\end{figure}



\DIFaddend \section{Data Applications}
We apply our ordinal tensor method to two real-world datasets. In the first application, we use our model to analyze an ordinal tensor consisting of structural connectivities among 68 brain regions for 136 individuals from Human Connectome Project (HCP)~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citep{geddes2016human}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{van2013wu}}%DIFAUXCMD
}\DIFaddend . In the second application, we perform tensor completion to an ordinal dataset with missing values. The data tensor records the ratings on a scale of 1 to 5 from 42 users to 139 songs on 26 contexts~\citep{baltrunas2011incarmusic}.


\DIFdelbegin \subsection{\DIFdel{Human Connectome Project (HCP)}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin {\bf \DIFadd{Human Connectome Project (HCP).}} \DIFaddend Each entry in the HCP dataset takes value on a nominal scale, \{{\it high, moderate, low}\}, indicating the strength level of fiber connection. We convert the dataset to a 3-level ordinal tensor $\tY\in[3]^{68\times 68\times 136}$ and apply the ordinal tensor method with a logistic link function. The BIC suggests $\mr = (23,23,8)$ with $\logl(\hat{\Theta},\hat{\mb}) = -216,646$. Based on the estimated Tucker factors $\{\hat \mM_k\}$, we perform a clustering analysis via K-mean on the brain nodes (see detailed procedure in \DIFdelbegin \DIFdel{the Supplement). We find that the }\DIFdelend \DIFaddbegin \DIFadd{Appendix~\ref{sec:clustering}). The }\DIFaddend 68 brain nodes are grouped into eight clusters\DIFdelbegin \DIFdel{, and }\DIFdelend \DIFaddbegin \DIFadd{. We find that }\DIFaddend the clustering capture the spatial separation \DIFdelbegin \DIFdel{of brain nodes very well (see the Supplement}\DIFdelend \DIFaddbegin \DIFadd{between brain regions (Supplementary Figure~\ref{figure:brain image}}\DIFaddend ). In particular, the top two clusters represent the left and right hemispheres; the smaller clusters represent local regions driving by similar nodes \DIFaddbegin \DIFadd{(Supplementary Table~\ref{table:clustering})}\DIFaddend . For example, the cluster \DIFaddbegin \DIFadd{III/}\DIFaddend IV consists of \DIFdelbegin \DIFdel{brain nodes all of which are called }%DIFDELCMD < {\it %%%
\DIFdel{R.SUPRAMARGINAL}%DIFDELCMD < } %%%
\DIFdel{and located at the same region . The identified similarity among brain nodes }\DIFdelend \DIFaddbegin \DIFadd{nodes in the supramarginal gyrus region in the left/right hemisphere. This region is known to be involved in visual word recognition and reading~\mbox{%DIFAUXCMD
\citep{stoeckel2009supramarginal}}%DIFAUXCMD
. The spatial separation between clusters }\DIFaddend suggests the applicability of our \DIFdelbegin \DIFdel{method.
}\DIFdelend \DIFaddbegin \DIFadd{clustering method without knowledge of external annotations. 
}\DIFaddend 

\DIFdelbegin \DIFdel{We compare the goodness-of-fit of various tensor methods on the HPC data. 
Table~\ref{table:CV} summarizes the prediction error via 5-fold stratified cross-validation averaged over 10 runs. Our method outperforms the others, especially in MAD.
}%DIFDELCMD < 

%DIFDELCMD < \begin{table}[http]
%DIFDELCMD < \begin{tabular}{c|c|c||c|c|c}
%DIFDELCMD < \Xhline{2\arrayrulewidth}
%DIFDELCMD < \multicolumn{3}{c||}{Human Connectome Project (HCP) dataset} & \multicolumn{3}{c}{InCarMusic dataset}\\
%DIFDELCMD < \Xhline{2\arrayrulewidth}
%DIFDELCMD < %%%
\DIFdelFL{Method                    }%DIFDELCMD < & %%%
\DIFdelFL{MAD }%DIFDELCMD < & %%%
\DIFdelFL{MCR }%DIFDELCMD < & %%%
\DIFdelFL{Method                    }%DIFDELCMD < & %%%
\DIFdelFL{MAD }%DIFDELCMD < & %%%
\DIFdelFL{MCR}%DIFDELCMD < \\
%DIFDELCMD < \hline
%DIFDELCMD < %%%
\DIFdelFL{Ordinal-T (ours) }%DIFDELCMD < & %%%
\DIFdelFL{0.1607 (0.0005)}%DIFDELCMD < & %%%
\DIFdelFL{0.1606 (0.0005)}%DIFDELCMD < & %%%
\DIFdelFL{Ordinal-T (ours) }%DIFDELCMD < & %%%
\DIFdelFL{1.37 (0.039) }%DIFDELCMD < &  %%%
\DIFdelFL{0.59 (0.009)}%DIFDELCMD < \\
%DIFDELCMD <  %%%
\DIFdelFL{Continuous-T}%DIFDELCMD < &%%%
\DIFdelFL{0.2530 (0.0002)}%DIFDELCMD < &%%%
\DIFdelFL{0.1599 (0.0002)}%DIFDELCMD < & %%%
\DIFdelFL{Continuous-T }%DIFDELCMD < & %%%
\DIFdelFL{2.39 (0.152)}%DIFDELCMD < &%%%
\DIFdelFL{0.94 (0.027)}%DIFDELCMD < \\
%DIFDELCMD < 

%DIFDELCMD <  %%%
\DIFdelFL{1bit-sign-T}%DIFDELCMD < &%%%
\DIFdelFL{0.3566 (0.0010)}%DIFDELCMD < &%%%
\DIFdelFL{0.1563 (0.0010)}%DIFDELCMD < & %%%
\DIFdelFL{1bit-sign-T}%DIFDELCMD < &%%%
\DIFdelFL{1.39 (0.003)}%DIFDELCMD < & %%%
\DIFdelFL{0.81 (0.005)}%DIFDELCMD < \\
%DIFDELCMD < \hline
...
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{Comparison of prediction error in the HPC and InCarMusic analyses. Standard errors are reported in parentheses.}}%DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{table:CV}%%%
%DIFDELCMD < \end{table}
%DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{InCarMusic recommendation system}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdelend \DIFaddbegin {\bf \DIFadd{InCarMusic recommendation system.}} \DIFaddend We apply ordinal tensor completion to a recommendation system \DIFdelbegin %DIFDELCMD < {\it %%%
\DIFdel{InCarMusic}%DIFDELCMD < }%%%
\DIFdel{. }%DIFDELCMD < {\it %%%
\DIFdel{InCarMusic }%DIFDELCMD < } %%%
\DIFdelend \DIFaddbegin \DIFadd{InCarMusic. InCarMusic }\DIFaddend is a mobile application that offers music recommendation to passengers of cars based on contexts~\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{baltrunas2011incarmusic}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{baltrunas2011incarmusic}}%DIFAUXCMD
}\DIFaddend . Our goal is to perform tensor completion to impute the unobserved entries in the $42\times 139\times 26$ ordinal tensor and thereby we can offer context-specific music recommendation to users. The data tensor consists of 2,884 observed entries. Table~\ref{table:CV} shows the averaged prediction error via 5-fold cross validation. The high missing rate makes the accurate classification challenging. Nevertheless, our method achieves the best performance among the three.
...
\caption{\DIFaddFL{Comparison of prediction error in the HPC and InCarMusic analyses. Standard errors are reported in parentheses.}}\label{table:CV}
\end{table}


\DIFaddend \section{\DIFdelbegin \DIFdel{Conclusions}\DIFdelend \DIFaddbegin \DIFadd{Proofs}\DIFaddend }\DIFdelbegin \DIFdel{We have developed a low-rank tensor estimation method based on possibly incomplete, ordinal-valued observations. A sharp error bound is established, and we demonstrate the outperformance of our approach compared to other methods. The work unlocks several directions of future research. One interesting question would be the inference problem, i.e.. to assess the uncertainty of the obtained estimates and the imputation. Other directions include the trade-off between (non)convex optimization and statistical/computational efficiency. While convex relaxations are popular approach for matrix-based problem, they are often slow in practice~\mbox{%DIFAUXCMD
\cite{chen2019non}}%DIFAUXCMD
.
The interplay between computational efficiency and statistical accuracy in general tensor problems warrants future research.
}%DIFDELCMD < 

%DIFDELCMD < %%%
\section{\DIFdel{Proofs}}%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdelend \label{sec:proof}
\DIFaddbegin \DIFadd{Here, we provide proofs of the theoretical results presented in Sections~\ref{sec:theory}.
}
...
\begin{proof}
Define $d_{\text{total}}=\prod_k d_k$. \DIFdelbegin \DIFdel{With a little }\DIFdelend \DIFaddbegin \DIFadd{By }\DIFaddend abuse of notation, we use $(\Theta,\mb)$ to denote \DIFdelbegin \DIFdel{a }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend length-$(d_{\text{total}}+L-1)$-vector collecting all parameters together. Let us denote a bivariate function
\begin{align}
...
\]
Then, $\lambda(u,v) = \lambda(\ma_1^T(\Theta,\mb),\ \ma_2^T(\Theta,\mb))$ is concave in $(\Theta,\mb)$ by the definition of concavity. \DIFdelbegin \DIFdel{Furthermore}\DIFdelend \DIFaddbegin \DIFadd{Therefore}\DIFaddend , we can conclude that $\logl(\Theta, \mb) $ is concave in $(\Theta,\mb)$ because $\logl(\Theta, \mb)$ is the sum of $\lambda(u,v)$. 

Now, we prove the concavity of $\lambda(u,v)$. Note that
\DIFdelbegin \begin{displaymath}
\DIFdel{\lambda(u,v) = \log\big[f(u)-f(v)\big]=\log\big[\int\mathds{1}_{[u,v]}(x)f'(x)dx\big],
}\end{displaymath}
%DIFAUXCMD
\DIFdelend \DIFaddbegin \begin{equation}
\DIFadd{\lambda(u,v) = \log\big[f(u)-f(v)\big]=\log\big[\int\mathds{1}_{[u,v]}(x)f'(x)dx\big],
}\end{equation}
\DIFaddend where $\mathds{1}_{[u,v]}$ is an indicator function that equals 1 in the interval $[u,v]$, and 0 elsewhere. Furthermore, $\mathds{1}_{[u,v]}(x)$ is log-concave in $(u,v,x)$\DIFdelbegin \DIFdel{and $f'(x) = \frac{e^x}{(1+e^x)^2}$ }\DIFdelend \DIFaddbegin \DIFadd{, and by Assumption~\ref{ass:link}, $f'(x)$ }\DIFaddend is log-concave in $x$. \DIFdelbegin \DIFdel{The convexities imply that $\mathds{1}_{(u,v)}(x)f'(x)$
}\DIFdelend \DIFaddbegin \DIFadd{It follows that $\mathds{1}_{[u,v]}(x)f'(x)$
}\DIFaddend is a log-concave in $(u,v,x)$. By Lemma~\ref{lem:lossconvexity}, we conclude that $\lambda(u,v)$ is concave in $(u,v)$ where $u>v$. 
\end{proof}
...
\DIFdelbegin \DIFdel{We begin with various notion of tensor norms }\DIFdelend \DIFaddbegin \DIFadd{This section collects lemmas }\DIFaddend that are useful for the proofs of the main theorems.

\begin{defn}[Atomic M-norm~\citep{ghadermarzy2019near}]
Define \DIFdelbegin \DIFdel{$T_{\pm}=\{\tT\in\{\pm 1 \}^{d_1\times \cdots \times d_K}\colon \text{rank}(\tT)=1\}$}\DIFdelend \DIFaddbegin \DIFadd{$T_{\pm}=\{\tT\in\{\pm 1 \}^{d_1\times \cdots \times d_K}\colon$ $\text{rank}(\tT)=1\}$}\DIFaddend . The atomic M-norm of a tensor $\Theta\in\mathbb{R}^{d_1\times \cdots \times d_K}$ is defined as
\begin{align}
...
\DIFdelbegin %DIFDELCMD < \begin{defn}[Absolutely continuous]
%DIFDELCMD < %%%
\DIFdel{A }\DIFdelend \DIFaddbegin \begin{defn}[Absolute continuity]
\DIFadd{We use $\lambda\ll\mu$ to indicate that a }\DIFaddend measure $\lambda$ is absolutely continuous with respect to another measure $\mu$\DIFdelbegin \DIFdel{if }\DIFdelend \DIFaddbegin \DIFadd{; that is, }\DIFaddend $\lambda(E)=0$ \DIFdelbegin \DIFdel{for every set with }\DIFdelend \DIFaddbegin \DIFadd{whenever }\DIFaddend $\mu(E)=0$\DIFdelbegin \DIFdel{and is denoted as $\lambda\ll\mu$}\DIFdelend . 
\end{defn}


\DIFdelbegin %DIFDELCMD < \begin{lem}[Theorem 2.5 in~\cite{tsybakov2009introduction}]%%%
\DIFdelend \DIFaddbegin \begin{lem}[Theorem 2.5 in~\cite{tsybakov2008introduction}]\DIFaddend \label{lem:Tsybakov}
...
\DIFadd{We have developed a low-rank tensor estimation method based on possibly incomplete, ordinal-valued observations. A sharp error bound is established, and we demonstrate the outperformance of our approach compared to other methods. The work unlocks several directions of future research. One interesting question would be the inference problem, i.e.. to assess the uncertainty of the obtained estimates and the imputation. Other directions include the trade-off between (non)convex optimization and statistical/computational efficiency. While convex relaxations are popular approach for matrix/tensor problem, they are often slow in practice~\mbox{%DIFAUXCMD
\citep{ge2017optimization,chen2019non}}%DIFAUXCMD
. The interplay between computational efficiency and statistical accuracy in general tensor problems warrants future research.
}

\section*{\DIFadd{Acknowledgments}}
\DIFadd{This research is supported in part by NSF grant DMS-1915978 and the University of Wisconsin-Madison, Office of the Vice Chancellor for Research and Graduate Education with funding from the Wisconsin Alumni Research Foundation. 
}\newpage
...
\DIFaddend \section{Extension of Theorem~\ref{thm:rate} to unknown cut-off points}\DIFaddbegin \label{sec:extention}
\DIFaddend 
...
\[
\DIFdelbegin \DIFdel{\text{MSE}}\DIFdelend \DIFaddbegin \DIFadd{\mathrm{MSE}}\DIFaddend (\hat \Theta, \trueT)\leq \DIFdelbegin %DIFDELCMD < {%%%
\DIFdel{c_1U_{\alpha,\beta,\Delta}}%DIFDELCMD < \over %%%
\DIFdel{L_{\alpha,\beta,\Delta}}%DIFDELCMD < } %%%
...
and
\[
\text{MSE}(\hat\mb, \trueb) \leq  \DIFdelbegin %DIFDELCMD < {%%%
\DIFdel{c_2 C_{\alpha,\beta,\Delta} }%DIFDELCMD < \over %%%
\DIFdel{D_{\alpha,\beta,\Delta}}%DIFDELCMD < }{%%%
\DIFdel{\max_{\ell}(n_\ell+n_{\ell+1})}%DIFDELCMD < \over %%%
\DIFdelend \min\DIFdelbegin \DIFdel{_{\ell}(n_{\ell}+n_{\ell+1})}%DIFDELCMD < }  %%%
...
\DIFaddbegin \DIFadd{The constant bounds $4\alpha^2$ and $4\beta^2$ follow trivially from the definition of the feasible sets. We focus on proving the non-constant bounds. }\DIFaddend Similar to the proof of Theorem~\ref{thm:rate}, we suppress $\Omega$ in the subscript. Based on the definition of $(\hat \Theta, \hat \mb)$,
...
\begin{align}\DIFdelbegin %DIFDELCMD < %DIFDELCMD < \label{eq:property}%%%
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \label{eq:propertyb}
\DIFaddend {\partial \flogl\over \partial b_\ell}&=\sum_{\omega\in\Omega}\left[\mathds{1}_{\{y_{\omega}=\ell\}}
...
%DIFAUXCMD
\addtocounter{section}{-1}%DIFAUXCMD
\DIFdel{In the main paper, we compare our method (}%DIFDELCMD < {\bf %%%
\DIFdel{Ordinal-T}%DIFDELCMD < }%%%
\DIFdel{) with other four methods (}%DIFDELCMD < {\bf %%%
\DIFdel{Ordinal-M}%DIFDELCMD < }%%%
\DIFdel{, }%DIFDELCMD < {\bf %%%
\DIFdel{1bit-category-T}%DIFDELCMD < }%%%
\DIFdel{,}%DIFDELCMD < {\bf %%%
\DIFdel{1bit-sign-T}%DIFDELCMD < }%%%
\DIFdel{, and }%DIFDELCMD < {\bf %%%
\DIFdel{Continuous-T}%DIFDELCMD < }%%%
\DIFdel{). The methods are evaluated by their capabilities in predicting the most likely label for each entry, i.e.,\ $y_\omega^{\text{mode}}=\arg\max_\ell\mathbb{P}(y_\omega=\ell)$. Here we perform additional simulations to compare the methods by their performance in predicting the median label, $y_\omega^{\text{median}}=\min\{\ell\colon\mathbb{P}(y_\omega=\ell)\geq 0.5\}$. Under the latent variable model~}%DIFDELCMD < \eqref{eq:latent} %%%
...
%DIFDELCMD <     \includegraphics[width=.6\textwidth]{panel3.pdf}
...
\DIFdelFL{Performance comparison in predicting the median label. (a,c) Prediction errors versus sample complexity $\rho=|\Omega|/d^k$ when $L=5$. (b,d) Prediction errors versus the number of ordinal levels $L$, when $\rho=0.8.$}}
  %DIFAUXCMD
%DIFDELCMD < %DIFDELCMD < \label{fig:compare2}%%%
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Figure~\ref{fig:compare2} compares the prediction accuracy under the setting $\alpha = 10, d = 20$ and $r = 5$. We see that our method outperforms the others in both MAD and MCR.  The methods with probability model (}%DIFDELCMD < {\bf %%%
\DIFdel{Ordinal-T, Ordinal-M, 1bit-category-T}%DIFDELCMD < }%%%
...
%DIFDELCMD < %%%
\DIFdelend \section{Additional results on HCP analysis}\DIFaddbegin \label{sec:additionalHCP}
\DIFaddend \subsection{Clustering based on Tucker representation}\label{sec:clustering}
...
X = U\Sigma V^T,
\end{equation}
where $\Sigma$ is a diagonal matrix and $ U,V$ are factor matrices with orthogonal columns. Second, we take each column of $V$ as a principal axis and \DIFaddbegin \DIFadd{each row in }\DIFaddend $U\Sigma$ as principal \DIFdelbegin \DIFdel{components}\DIFdelend \DIFaddbegin \DIFadd{component}\DIFaddend . A subsequent multivariate clustering method (such as $K$-means) is then applied to the $m$ rows of $U\Sigma$. 


We apply a similar clustering procedure to the estimated parameter tensor $\hat\Theta$. Based on Tucker \DIFdelbegin \DIFdel{decomposition}\DIFdelend \DIFaddbegin \DIFadd{representation of $\hat \Theta$}\DIFaddend , we have
...
\end{equation}
where $\hat \Theta_{(k)} , \hat \tC_{(k)}$ denote the mode-$k$ unfolding of $\hat \Theta$ and $\hat \tC$, respectively. Then, the mode-$k$ clustering can be performed as follows. First, we take \DIFaddbegin \DIFadd{columns in }\DIFaddend $\left(\hat \mM_K\otimes\cdots\otimes\hat \mM_1\right)$ as principal axes and \DIFaddbegin \DIFadd{rows in }\DIFaddend $\hat \mM_k\hat \tC_{(k)}$ as principal components. Then, we perform $K$-means clustering method to the $d_k$ rows of the \DIFdelbegin \DIFdel{principal components }\DIFdelend \DIFaddbegin \DIFadd{matrix }\DIFaddend $\hat \mM_k\hat \tC_{(k)}$. 

\subsection{Clustering results on HCP}
We perform \DIFdelbegin \DIFdel{clustering analysis to }\DIFdelend \DIFaddbegin \DIFadd{a clustering analysis on }\DIFaddend the 68 brain nodes using the the produce described in Section~\ref{sec:clustering}. Recall that our ordinal tensor method outputs the estimated parameter tensor $\hat\Theta\in\mathbb{R}^{68\times 68\times136}$ with rank $(23,23,8)$. \DIFdelbegin \DIFdel{The number of clusters in the }\DIFdelend \DIFaddbegin \DIFadd{We apply }\DIFaddend $K$-means \DIFdelbegin \DIFdel{is chosen based on the elbow method }\DIFdelend \DIFaddbegin \DIFadd{to the mode-1 principal component matrix of size $68\times 184$ }\DIFaddend (\DIFdelbegin \DIFdel{see figure}\DIFdelend \DIFaddbegin \DIFadd{$184=23\times8$). The elbow method suggests eight clusters among the 68 nodes (see Figure}\DIFaddend ~\ref{figure:elbow}). Table~\ref{table:clustering} shows the eight clusters \DIFdelbegin \DIFdel{of brain nodes. }%DIFDELCMD < \begin{figure}[H]
%DIFDELCMD <   %DIFDELCMD < \label{figure:elbow}%%%
%DIFDELCMD <   \begin{center}
%DIFDELCMD <     \includegraphics[width=0.49\textwidth]{elbowm.pdf}
%DIFDELCMD <   \end{center}
%DIFDELCMD <   %%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{The elbow plot for the number of clusters}}
%DIFAUXCMD
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{and their compositions. }\DIFaddend We find that most brain nodes fall into cluster I and the cluster II, which can be represented as the left and right hemispheres, respectively (Figure~\ref{figure:brain image}a). \DIFdelbegin \DIFdel{From cluster III to the cluster VIII, entries within each cluster share the same name encoded in the data}\DIFdelend \DIFaddbegin \DIFadd{Clusters III-VIII captures brain regions with same annotations in the dataset}\DIFaddend . For example, \DIFdelbegin \DIFdel{cluster IIIand cluster IV represent the }%DIFDELCMD < {\it %%%
\DIFdel{Supramarginal gyrus }%DIFDELCMD < } %%%
\DIFdelend \DIFaddbegin \DIFadd{clusters III/IV represents the supramarginal gyrus }\DIFaddend region on the left\DIFdelbegin \DIFdel{brain and right brain. These regions are }\DIFdelend \DIFaddbegin \DIFadd{/right hemisphere, respectively. This region is }\DIFaddend known to be \DIFdelbegin \DIFdel{in charge of interpreting tactile sensory data and involved in perception of space and limbs location~\mbox{%DIFAUXCMD
\cite{carlson2012physiology,reed1994nature}}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{involved in visual word recognition and reading~\mbox{%DIFAUXCMD
\citep{stoeckel2009supramarginal}}%DIFAUXCMD
}\DIFaddend . The results demonstrate that our clustering successfully groups similar brain \DIFdelbegin \DIFdel{nodes }\DIFdelend \DIFaddbegin \DIFadd{regions }\DIFaddend together without knowledge of external annotation.

\DIFdelbegin %DIFDELCMD < \begin{table}[H]
%DIFDELCMD <   %DIFDELCMD < \label{table:clustering}%%%
%DIFDELCMD <   %%%
...
\DIFdelendFL \DIFaddbeginFL \begin{figure}[ht]
 ...
 \DIFdelbeginFL %DIFDELCMD < \includegraphics[width = 0.45\textwidth]{braincluster2.pdf}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width = 0.45\textwidth]{braincluster3.pdf}
  \caption{\DIFaddFL{Top two clusters of brain nodes. (a) Nodes in yellow are from cluster I (left hemisphere), and nodes in green are from cluster II (right hemisphere). (b) Predicted connectivity within cluster I and cluster II. Edges are colored based on predicted strength level averaged across individuals.}}  \DIFaddendFL \label{figure:brain image}
\DIFdelbeginFL %DIFDELCMD < \caption{%
{%DIFAUXCMD
\DIFdelFL{In the left brain image, yellow nodes are from  cluster I and Green nodes are from cluster II.  Nodes from each cluster from III to VIII have almost same locations. The right brain image shows the extent of connectivity among nodes in each cluster I and cluster II. We take average of connections among nodes over 136 individuals. Yellow color means strong connectivity and green color weak connectivity. }}
%DIFAUXCMD
\DIFdelendFL \end{figure}


\DIFdelbegin %DIFDELCMD < \bibliographystyle{plainnat}
%DIFDELCMD < \bibliography{tensor_wang}
%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \begin{figure}[H]
\begin{center}
 \includegraphics[width=0.49\textwidth]{elbowm.pdf}
\end{center}
 \caption{\DIFaddFL{Elbow plot for determining the number of clusters in $K$-means.}}\label{figure:elbow}
\end{figure}
\DIFaddend 



\DIFaddbegin \bibliography{tensor_wang}
