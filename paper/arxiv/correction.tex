...
\]
where $\mathbf{1}$ denotes a length-$(L-1)$ vector of all ones. Under the identifiability constraint $\langle \Theta, \tJ \rangle=0$, there is an one-to-one mapping between $\tZ$ and $(\Theta, \mb)$, with $\text{rank}(\tZ)=(\text{rank}(\Theta),2)^T$. Furthermore, 
...
\]
Furthermore, properties (b) and (c) (or similar arguments as in page 29, arXiv preprint) implies that
\begin{equation}
