...
&=\sum_{\omega}\theta^2_\omega+\min_\ell\left(n_{\ell}+n_{\ell+1}\right)\sum_{\ell}b_\ell^2
\end{align}
...
Therefore, there exists a positive constant $c_1>0$ (? perhaps depending on $\alpha, \beta,\Delta$ etc...needs verification..) such that
\[
{{\mz^T\mH\mz}\over \FnormSize{}{\mz}^2}\geq c_1 {\min_{\ell}(n_\ell+n_{\ell_1})\over (L-1)\prod_k d_k}.
\]
}
\end{enumerate}
\end{proof}
