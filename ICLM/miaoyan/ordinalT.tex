...
An efficient completion method is further developed, which guarantees the consistent recovery of an order-$K$ $(d,\ldots,d)$-dimensional tensor using only $O(Kd)$ noisy, quantized observations. We demonstrate the outperformance of our approach over previous methods through applications to the tasks of clustering and collaborative filtering. 
...
{\color{red}(Prior work and our contribution. See table~\eqref{tab:compare} for  comparison.)}
...
\end{thm}
Theorem~\ref{thm:rate} provides the statistical converge for the estimator~\eqref{eq:estimator}. Its proof is given in the Supplements. In fact, the proof of this theorem shows that the same statistical rate holds not only for the global optimizer of~\eqref{eq:estimator}, but also for any local optimizer $\check \Theta$ in the level set $\{\check\Theta\in\tP: \logl(\check\Theta)\geq \logl(\trueT)\}$. This suggests that the local optimality itself is not necessarily a severe concern in our context, as long as the objective value at the estimator is large enough. {\color{red}In Section~\ref{algorithm}, we will provide an algorithm-dependent rate quantifying the interplay between the statistical and computational errors.} 

A similar conclusion can be obtained for the prediction error, measured in KL divergence, between the two generative distributions. 
\begin{cor}[Prediction error]~\label{cor:prediction}
Assume the same set-up as in Theorem~\ref{thm:rate}. Let $\mathbb{P}_{\tY}$ and $\hat{\mathbb{P}}_{\tY}$ denote the distributions generating the $L$-level ($L\geq 2$) ordinal tensor $\tY$, given the true parameter $\Theta$ and its estimator $\hat \Theta$, respectively. Then, with very high probability, 
...
Intuitively, for tensor data analysis, we can view the number of entries, $\prod_k d_k$, as the sample size, and the number of free parameters in $\tP$ is roughly on the order of $\sum_{k}d_k$, assuming $r_{\max}=O(1)$. Compared to the matrix case, a higher tensor order implies more effective samples per parameter and thus exhibits a faster convergence rate in high dimensions. 


We next establish the statistical optimality of our estimator $\hat \Theta$. The result is based on the information theory, and therefore applies to all estimators in $\tP$, including but not limited to $\hat \Theta$ in~\eqref{eq:estimator}. We show that this lower bound nearly matches the upper bound in~\eqref{eq:rate}, as the tensor dimension increases. 


\begin{thm}[Minimax lower bound]\label{thm:minimax} 
Assume the same set-up as in Theorem~\ref{thm:rate}, and $d_{\max}=\max_k d_k \geq 8$. Let $\inf_{\hat \Theta}$ denote the infimum over all estimators $\hat \tA\in\tP$ based on the ordinal tensor observation $\tY\in[L]^{d_1\times \cdots \times d_K}$. Then, under the model~\eqref{eq:model},
...
\end{align}
where $C=C(\alpha, L, f,\mb)>0$ is a constant independent of the tensor dimension and rank. 
\end{thm}


\subsection{Sample complexity for tensor completion}

Now we consider the problem of tensor completion, when only a subset of entries $\Omega$ are observed. We consider a general sampling procedure induced by $\Pi$, and assess the accuracy using the weighted squared deviation:
...
Note that the recovery error depends on the distribution. In particular, the entries with higher sampling probabilities are equipped with better accuracy guarantee, compared to the ones with lower sampling probabilities. 
