...
%DIF DEL ordinalT_supp.tex      Tue Jan 21 18:00:59 2020
%DIF ADD ordinalT_supp_v2.tex   Mon Feb  3 20:54:25 2020
...
Define $d_{\text{total}}=\prod_k d_k$. Let \DIFdelbegin \DIFdel{$\fplogl\in\mathbb{R}^{d_\text{total}}$ denote the vector }\DIFdelend \DIFaddbegin \DIFadd{$\fplogl\in\mathbb{R}^{d_1\times\cdots\times d_K}$ denote the tensor }\DIFaddend of gradient with respect to \DIFdelbegin \DIFdel{$\Vec(\Theta)\in\mathbb{R}^{d_{\text{total}}}$}\DIFdelend \DIFaddbegin \DIFadd{$\Theta\in\mathbb{R}^{d_1\times \cdots\times d_K}$}\DIFaddend , and $\fpplogl$ the corresponding Hession matrix of size $d_\text{total}$-by-$d_{\text{total}}$. Here, $\Vec(\cdot)$ denotes the operation that turns a tensor into a vector. By~\eqref{eq:property}, $\fpplogl$ is a diagonal matrix. Recall that
\[
U_\alpha=\max\DIFdelbegin \DIFdel{_{\ell\in[L],|\alpha|\leq \alpha}{\dot{g}_\ell(\theta)\over g_\ell(\theta)}}\DIFdelend \DIFaddbegin \DIFadd{_{\ell\in[L],|\theta|\leq \alpha}{\dot{g}_\ell(\theta)\over g_\ell(\theta)}}\DIFaddend >0 \quad \text{and}\quad
L_\alpha=\DIFdelbegin \DIFdel{\max_{\ell\in[L],|\alpha|\leq \alpha} }\DIFdelend \DIFaddbegin \DIFadd{\min_{\ell\in[L],|\theta|\leq \alpha} }\DIFaddend {\dot{g}^2_\ell(\theta)-\ddot{g}_\ell(\theta)g_\ell(\theta)\over g^2_\ell(\theta)}>0.
\]
...
\section{\DIFadd{Convexity of the likelihood function}}
\begin{thm}\label{thm:convexity}
\begin{align}
 \DIFadd{\logl(\Theta, \mb)}&\DIFadd{=\sum_{\omega\in\Omega}\sum_{\ell\in[L]} \Big\{\mathds{1}_{\{y_\omega=\ell\}} \log \big[f(\theta_\omega+b_\ell)-  f(\theta_\omega+b_{\ell-1})\big]\Big\}, \text{ where } f(x) = \frac{e^x}{1+e^x}.
 }\end{align}
\DIFadd{is a concave function to $(\Theta,\mb)$
}\end{thm}
\begin{proof}
\DIFadd{We use $(\Theta,\mb)$ abusively as $(Vec(\Theta)^T,\mb^T)^T$ in this proof.
...
}$$\DIFadd{u = \ma_1^T(\Theta,\mb), v = \ma_2^T(\Theta,\mb) \text{ for some }\ma_1,\ma_2\in \mathbb{R}^{d_{\text{total}}+L-1},}$$ \DIFadd{then $\lambda(u,v) = \lambda(\ma_1^T(\Theta,\mb),\ma_2^T(\Theta,\mb)))$ is a concave function to $(\Theta,\mb)$
...
\DIFadd{Notice that  $\mathds{1}_{(u,v)}(x)$ is log-concave to $(u,v,x)$ and $f'(x)$ is log-concave to $x$ because $f'(x) = \frac{e^x}{(1+e^x)^2}$. Those convexities imply that $\mathds{1}_{(u,v)}(x)f'(x)$
is a log-concave function to $(u,v,x)$. Therefore, we can conclude that the $\lambda(u,v)$ is a concave function from the lemma~\ref{lem:convexity}.
}\end{proof}

\begin{lem}[Corollary 3.5 in~\cite{brascamp2002extensions}]\label{lem:convexity}
\DIFadd{Let $F(x,y):\mathbb{R}^{m+n}\rightarrow \mathbb{R}$ be an integrable function where $x\in \mathbb{R}^{m},y\in \mathbb{R}^n$. Let }$$\DIFadd{G(x) = \int_{\mathbb{R}^n}F(x,y)dy.}$$ \DIFadd{If $F(x,y)$ is a log concave function to (x,y), then $G(x)$ is a log concave function.
}\end{lem}

\section{\DIFadd{Brain clustering method and result}}
\subsection{\DIFadd{Clustering method}}
...
\DIFadd{\Theta = \tC\times_1\mM_1\times_2\cdots\times_K\mM_K,
}\end{equation*}
\DIFadd{where $\tC\in\mathbb{R}^{r_1\times \cdots \times r_K}$ is a core tensor, $\mM_k\in\mathbb{R}^{d_k\times r_k}$ are factor matrices with orthogonal columns, and $\times_k$ denotes the tensor-by-matrix multiplication~\mbox{%DIFAUXCMD
\cite{kolda2009tensor}}%DIFAUXCMD
. We perform mode-$k$ matricization of the tensor $\Theta$ to cluster data from mode-$k$ variable as follows.
}\begin{equation*}
\DIFadd{\Theta_{(k)} = \mM_k\tC_{(k)}\big(\mM_K\otimes\cdots\otimes\mM_1\big)
}\end{equation*}
\DIFadd{We interpret $\big(\mM_K\otimes\cdots\otimes\mM_1\big)$ as principal axes and $\mM_k\tC_{(k)}$ as principal components. By putting other mode variables as principal axes, we can exclude effects from those variables and can see $k$-th mode effect from checking principal components, $\mM_k\tC_{(k)}$. Therefore, we perform $K$-means method to $d_k$ rows of the principal components like in the matrices case.
}

\subsection{\DIFadd{Clustering result on HCP}}
\DIFadd{We perform $K$-means method to cluster brain nodes based on the suggested above method. We have the estimated latent parameter $\hat\Theta$ with the rank $(23,23,8)$ which minimizes the BIC value. The number of clusters is chosen to be eight from the elbow method in figure ~\ref{figure:elbow}.
...
\hline
\DIFaddFL{Group       }& \multicolumn{3}{c|}{\#1}                                                  & \multicolumn{3}{c|}{\#2}                                      \\ \hline
...
\DIFaddFL{Group       }& \DIFaddFL{\#3    }& \DIFaddFL{\#4    }& \DIFaddFL{\#5   }& \DIFaddFL{\#6  }& \DIFaddFL{\#7 }& \DIFaddFL{\#8
}\\ \hline
\DIFaddFL{Brain nodes }& \begin{tabular}[c]{@{}c@{}}\DIFaddFL{"SupraM\_L" }\\ \DIFaddFL{"SupraM\_L" }\\ \DIFaddFL{"SupraM\_L" }\\ \DIFaddFL{"SupraM\_L"}\end{tabular}                                      & \begin{tabular}[c]{@{}c@{}}\DIFaddFL{"SupraM\_R" }\\ \DIFaddFL{"SupraM\_R" }\\ \DIFaddFL{"SupraM\_R" }\\ \DIFaddFL{"SupraM\_R"}\end{tabular}                                      & \begin{tabular}[c]{@{}c@{}}\DIFaddFL{"IT\_L" }\\ \DIFaddFL{"IT\_L" }\\ \DIFaddFL{"IT\_L"}\end{tabular}                                      & \begin{tabular}[c]{@{}c@{}}\DIFaddFL{"IT\_R" }\\ \DIFaddFL{"IT\_R" }\\ \DIFaddFL{"IT\_R"}\end{tabular}                                                          & \begin{tabular}[c]{@{}c@{}}\DIFaddFL{"MT\_L" }\\ \DIFaddFL{"MT\_L" }\\ \DIFaddFL{"MT\_L"}\end{tabular}                                                         & \begin{tabular}[c]{@{}c@{}}\DIFaddFL{"MT\_R" }\\ \DIFaddFL{"MT\_R"}\\ \DIFaddFL{"MT\_R"}\end{tabular}                                                         \\ \hline
\end{tabular}
\end{sc}
\end{small}
\caption{\DIFaddFL{Result of the brain nodes clustering: the last alphabet in each node name means the left side for `L' and the right side for `R' in the brain}}
\end{table}
...
\begin{figure}[H]
 ...
 \includegraphics[width = 0.45\textwidth]{brain4.jpg}
  ...
  \caption{\DIFaddFL{Brain image of eight clusters: entries of each cluster are connected by edges}}
\end{figure}
\DIFaddend 
