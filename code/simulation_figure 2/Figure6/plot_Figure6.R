...
################ plot ############
p=ggplot(data=Output,aes(x=rho,y=MAD))+geom_line(aes(color=method),size=1.2)+geom_point(aes(shape=method))+ labs(x=expression('Observation fraction'~(abs(Omega)/d^K)), y='MAD')+theme(text = element_text(size=rel(4)),legend.text = element_text(size = 15),axis.text.x = element_text(size=15),axis.text.y = element_text(size=15))+ylim(0,2.1)

g <- ggplot_build(p)
col=unlist(unique(g$data[[1]]["colour"]))[c(5,3,4,1,2)]

p=p+geom_errorbar(aes(ymin=MAD-sd, ymax=MAD+sd), width=0.008,
position=position_dodge(0.05),color=col[rep(rep(1:5),length(rholist))],size=0.5)+geom_point(aes(shape=method),size=1.5)

pdf("MAD_mode.pdf",width=6.2,height=4)
p
dev.off()
